<template>
  <q-dialog
    v-if="id!==null"
    v-model="show"
  >
    <q-card
      style="width: 700px; max-width: 80vw;"
      class="q-pa-md"
    >
      <q-card-section class="row items-center">
        <div class="text-h6">
          Редактирование пользователя
        </div>
        <q-space />
        <q-btn
          v-close-popup
          icon="close"
          flat
          round
          dense
        />
      </q-card-section>

      <q-card-section class="q-gutter-lg">
        <q-input
          v-model="form.name"
          label="Имя *"
          hint="Ваше имя"
          :error="$v.form.name.$error"
          @blur="$v.form.name.$touch()"
          @input="clearServerError('name')"
        >
          <template v-slot:error>
            <span v-if="!$v.form.name.required">Укажите ваше имя. </span>
            <span v-if="!$v.form.name.minLength">Имя должно быть не короче {{ $v.form.name.$params.minLength.min }} символов</span>
            <span v-if="!$v.form.name.server">{{ errors.get('name').join(' ') }}</span>
          </template>
        </q-input>
        <q-input
          v-model="form.surname"
          label="Фамилия *"
          hint="Ваша фамилия"
          :error="$v.form.surname.$error"
          @blur="$v.form.surname.$touch()"
          @input="clearServerError('surname')"
        >
          <template v-slot:error>
            <span v-if="!$v.form.surname.required">Укажите вашу фамилию. </span>
            <span v-if="!$v.form.surname.server">{{ errors.get('surname').join(' ') }}</span>
          </template>
        </q-input>
        <q-input
          v-model="form.email"
          type="email"
          label="Email *"
          hint="Ваш адрес электронной почты"
          :error="$v.form.email.$error"
          autocomplete="new-email"
          @blur="$v.form.email.$touch()"
          @input="clearServerError('email')"
        >
          <template v-slot:error>
            <span v-if="!$v.form.email.required">Укажите адрес электронной почты. </span>
            <span v-if="!$v.form.email.email">Укажите подходящий адрес электронной почты </span>
            <span v-if="!$v.form.email.server">{{ errors.get('email').join(' ') }}</span>
          </template>
        </q-input>
        <!--        <q-input-->
        <!--          v-model="form.phone"-->
        <!--          label="Телефон"-->
        <!--        />-->
        <VuePhoneNumberInput
          v-model="form.phone"
          no-flags
          no-country-selector
          :border-radius="0"
          default-country-code="RU"
          :translations="{phoneNumberLabel: 'Номер телефона',example: 'Пример: '}"
          @update="phoneUpdate"
          @phone-number-blur="blurPhone"
        />
        <q-input
          v-model="form.password"
          type="password"
          label="Пароль"
          hint="Введите пароль"
          :error="$v.form.password.$error"
          autocomplete="new-password"
          @blur="$v.form.password.$touch()"
          @input="clearServerError('password')"
        >
          <template v-slot:prepend>
            <q-icon name="vpn_key" />
          </template>
          <template v-slot:error>
            <span v-if="!$v.form.password.required">Укажите пароль. </span>
            <span v-if="!$v.form.password.minLength">Пароль должен быть не короче {{ $v.form.password.$params.minLength.min }} символов</span>
            <span v-if="!$v.form.password.server">{{ errors.get('password').join(' ') }}</span>
          </template>
        </q-input>

        <q-select
          v-model="form.roles"
          multiple
          :options="rolesOptions"
          label="Роли пользователя *"
          use-chips
          emit-value
          map-options
          :error="$v.form.roles.$error"
          @blur="$v.form.roles.$touch()"
        >
          <template v-slot:error>
            <span v-if="!$v.form.roles.required">Выберите хотя бы одну роль. </span>
          </template>
        </q-select>
        <!--        <q-select-->
        <!--          v-model="form.permissions"-->
        <!--          multiple-->
        <!--          :options="permissionsOptions"-->
        <!--          label="Разрешения пользователя *"-->
        <!--          use-chips-->
        <!--          emit-value-->
        <!--          map-options-->
        <!--          :error="$v.form.permissions.$error"-->
        <!--          @blur="$v.form.permissions.$touch()"-->
        <!--        >-->
        <!--          <template v-slot:error>-->
        <!--            <span v-if="!$v.form.permissions.required">Выберите хотя бы одно разрешение. </span>-->
        <!--          </template>-->
        <!--        </q-select>-->
        <q-toggle
          v-model="form.isDeleted"
          label="Удалён"
          color="negative"
          :false-value="0"
          :true-value="1"
        />
        <q-toggle
          v-model="form.isActivated"
          label="Активирован"
          color="positive"
          :false-value="0"
          :true-value="1"
        />
        <q-toggle
          v-model="form.isBlocked"
          label="Заблокирован"
          color="negative"
          :false-value="0"
          :true-value="1"
        />
        <q-btn-toggle
          v-model="form.type"
          no-caps
          push
          spread
          class="custom-spread"
          toggle-color="primary"
          :options="[
            {label: 'Агент, работающий в агентстве', value: 'agencyAgent'},
            {label: 'Частный агент', value: 'privateAgent'},
            {label: 'Собственник', value: 'owner'},
          ]"
        />

        <ChooseAgency
          v-model="form.agency"
          :disabled="form.type!=='agencyAgent'"
          :error="$v.form.agency.$error"
          @blur="$v.form.agency.$touch()"
        />
      </q-card-section>
      <q-card-actions align="right">
        <q-btn
          outline
          label="Сохранить"
          :loading="processing"
          :disable="processing"
          icon-right="send"
          color="primary"
          @click="updateUser"
        />
        <q-btn
          outline
          :loading="processing"
          :disable="processing"
          label="Отмена"
          icon-right="not_interested"
          @click="show=false"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>

<script>
  import { required, minLength, email, requiredIf } from 'vuelidate/lib/validators'
  import serverError from 'src/mixins/serverError'
  import { mapFields } from 'vuex-map-fields'
  import ChooseAgency from "components/agencies/ChooseAgency"
  import VuePhoneNumberInput from 'vue-phone-number-input';
  import 'vue-phone-number-input/dist/vue-phone-number-input.css';
    export default {
    name: 'EditUser',
    components: {ChooseAgency, VuePhoneNumberInput},
    mixins:[serverError],
    props:{
      value:Boolean,
      id:
        {
              required:true
        }
  },
    data () {
      return {
        user:null,
        formattedPhone:null,
        processing:false,
        rolesOptions:[],
        //permissionsOptions:[],
        form:{id:null,email:null, password:null, name:null, phone:null, surname:null, agency:null, type:null, roles:[], isDeleted:null, isBlocked:null, isActivated:null},
      }
    },

    computed:{
      ...mapFields(
        'admin',
        ['roles']
      ),
      show: {
        get: function() {
          return this.value
        },
        set: function(value) {
          this.$emit('input', value)
          console.log(value)

        }
      }
    },
    watch : {
      value(newVal){
        if(newVal===false){
          this.resetForm()
          this.$v.$reset()
        }else{
          this.user=this.$store.getters['admin/getUser'](this.id)
          console.log(this.user)
          if (typeof this.user.profile !== 'undefined'&&this.user.profile!==null) {
            this.form.name=this.user.profile.name
            this.form.surname=this.user.profile.surname
            this.form.phone=this.user.profile.phone_mobile
            this.form.type=this.user.profile.type
          }
          this.form.email=this.user.email
          this.form.id=this.user.id

          if (typeof this.user.agency !== 'undefined'&&this.user.agency!==null) {
            this.form.agency=this.user.agency.id
          }
          if (typeof this.user.roles !== 'undefined'&&this.user.roles!==null) {
            this.form.roles=this.user.roles.map(a => a.item_name)
          }
          // if (typeof this.user.permissions !== 'undefined'&&this.user.permissions!==null) {
          //   this.form.permissions=Object.keys(this.user.permissions)
          // }

          this.form.isDeleted=this.user.isDeleted
          this.form.isBlocked=this.user.blocked_at!==null?1:0
          this.form.isActivated=this.user.confirmed_at!==null?1:0



        }
      }
    },
    methods:{
      phoneUpdate(value){
        this.formattedPhone=value.formatInternational
      },
      blurPhone(){
        this.form.phone=this.formattedPhone
      },
      resetForm(){
        this.form={id:null,email:null, password:null, name:null, phone:null, surname:null, agency:null, type:null, roles:[], permissions:[], isDeleted:null, isBlocked:null, isActivated:null}
      },
      loadRoles(){
        this.$store.dispatch('admin/getRoles').then((response) => {
          console.log(response)
          this.rolesOptions=Object.keys(response.data)
        })
      },
      loadPermissions(){
        this.$store.dispatch('admin/getPermissions').then((response) => {
          console.log(response)
          Object.keys(response.data).map(key=> {
            if(response.data[key].description!==null){
              this.permissionsOptions.push({label:response.data[key].description, value:key})
            }else{
              this.permissionsOptions.push({label:key, value:key})
            }
          })
        })
      },
      updateUser(){
        this.$v.form.$touch()
        if (this.$v.form.$invalid) {
          return
        }
        this.processing = true

        this.$store.dispatch('admin/editUser', this.form).then(response => {
          console.log(response)
          this.$q.notify('Пользователь успешно отредактирован')
          this.$v.$reset()
          this.show=false
        })
          .catch((err) => {
            console.log(err)
            console.log(err.response)
            this.$q.notify(err.response.data.message)
            this.collectErrors(err.response.data.errors)
          })
          .finally(()=>{
            this.processing = false
          })
      }
    },
    mounted () {
      this.loadRoles()
      //this.loadPermissions()
    },
    validations: {
      form: {
        agency: {
          requiredIf: requiredIf(function() {
            return this.form.type === 'agencyAgent' && !this.form.noAgency
          })
        },
        email: {
          email,
          required,
          server: function () {
            return !this.hasServerError('email')
          }
        },
        roles:{
          required,
          server: function () {
            return !this.hasServerError('roles')
          }
        },
        // permissions:{
        //   required,
        //   server: function () {
        //     return !this.hasServerError('permissions')
        //   }
        // },
        name: {
          minLength: minLength(2),
          required,
          server: function () {
            return !this.hasServerError('name')
          }
        },
        password: {
          minLength: minLength(6),
          server: function () {
            return !this.hasServerError('password')
          }
        },
        surname: {
          required,
          server: function () {
            return !this.hasServerError('surname')
          }
        }
      }

    }
  }
</script>
<style lang="stylus">
  .custom-spread button:first-child
    flex: 18000 1 0%
</style>
